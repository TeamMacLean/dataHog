      ...
      projects.map(function (project) {
        if (project === 'additional') {
          ...
          additionalFiles.map(function (af) {
            new AdditionalFile({parentID: rGroup.id, name: af}).save().then(function (rAdditionalFile) {
            }).error(function (err) {
              throw err;
            });
          });
        ...
        new Project({
          ...
          samples.map(function (sample) {
            if (sample === 'additional') {
              ...
              var additionalFiles = getFiles(fullPath);
              additionalFiles.map(function (af) {
                new AdditionalFile({parentID: rProject.id, name: af}).save().then(function (rAdditionalFile) {
                }).error(function (err) {
                  throw err;
                });
              });
            ...
            new Sample({
              ...
              runs.map(function (run) {
                ...
                if (run === 'additional') {
                  ...
                  var additionalFiles = getFiles(fullPath);
                  additionalFiles.map(function (af) {
                    new AdditionalFile({parentID: rSample.id, name: af}).save().then(function (rAdditionalFile) {
                    }).error(function (err) {
                      throw err;
                    });
                  });
                ...
                new Run({
                  ...
                  additionalFiles.map(function (aFile) {
                    new AdditionalFile({parentID: rRun.id, name: aFile}).save().then(function (rAdditionalFile) {
