...
function fastqcGen(filePath, cb) {
  var parentFolder = path.dirname(filePath);
  var fqcPath = path.join(parentFolder, '.fastqc');

  if (filePath.indexOf('.fq') > -1 || filePath.indexOf('.fastq') > -1) {
    ...
    //      }
    //    } else {
    //      fastqc.run(filePath, fqcPath, function (err) {
    //        if (cb) {
    //          cb(err);
    //        }
    //      });
...
function getSibling(pairs, myFileName) {
  ...
  if (pairs && pairs.length > 0) {
    pairs.map(function (pair) {
      console.log(pair, myFileName);
      if (pair[0] == myFileName) {
        console.log('paired', pair[0], pair[1], myFileName);
        return pair[1];
                                  ...
                                  if (err) {
                                    ...
                                    async.eachSeries(raws, function iterator(raw, nextRaw) {
                                      ...
                                      util.md5Stream(fullPath, function (md5) {
                                        ...
                                        var sibling = getSibling(pairs, raw);
                                ...
                                getPairs(path.join(processedPath, 'pairs.txt'), function (err, pairs) {
                                  if (err) {
                                    ...
                                    async.eachSeries(processeds, function iterator(processed, nextProcessed) {
                                      ...
                                      util.md5Stream(fullPath, function (md5) {
                                        var sibling = getSibling(pairs, processed);
                                        console.log('sibling is', sibling);
                                        new Read({
                                          processed: true,
                                          runID: rRun.id,
